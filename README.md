<p align="center"><img src="https://cdn.jsdelivr.net/gh/tagproject/docs-shared-config/media/banner.svg" alt="Package logo"></p>

<p align="center">
    <a href="https://github.com/tagproject/docs-shared-config/actions"><img src="https://github.com/tagproject/docs-shared-config/actions/workflows/build.yml/badge.svg" alt="Build Status"></a>
    <a href="https://www.npmjs.com/package/@tagproject/docs-shared-config"><img alt="npm" src="https://img.shields.io/npm/v/@tagproject/docs-shared-config.svg"></a>
    <a href="https://github.com/keindev/standard-shared-config"><img src="https://img.shields.io/badge/standard--shared--config-config-green?logo=data:image/png;base64," alt="Standard Shared Config"></a>
</p>

[Standard Shared Config](https://github.com/keindev/standard-shared-config) with docs for `tagproject` packages

## Install

```console
npm install @tagproject/docs-shared-config --save-dev
```

## Usage

- Add `"prepare:config": "docs-shared-config"` to `scripts` property in your `package.json`
- Rename your `prepare` scripts to `prepare:[NAME]`
- Add `"prepare": "npm-run-all prepare:*"`
- Run `npm run prepare`

> use `node --experimental-specifier-resolution=node node_modules/.bin/docs-shared-config` while [ESM](https://nodejs.org/api/esm.html) is in `Experimental`
